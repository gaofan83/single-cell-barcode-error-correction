...
function usage()
{
    echo "./barcode_correct.sh [options]"
    echo "-h --help"
    echo "Please specify the following options:"
    echo "-fq1 --fastq_cell=$READ_CELL_INDEX   [gzipped fastq R1 or R2 for cell index reads]"
    echo "-fq2 --fastq_biology=$READ_BIOLOGY   [gzipped fastq R1 or R2 for biology reads]"
    ...
    echo "-w --whitelist=$WHITELIST [whitelist of barcodes, optional]"
    echo ""
    echo "Example below:"
    echo "bash barcode_correct4.sh -fq1=SRR1853178_1.fastq -fq2=SRR1853178_2.fastq -bcs=1 -bcl=12 -t=8 -o=BCFIX"
}
